...
export default function Register() {
  ...
  const onSubmit = async (data) => { await authService.register(data.email, data.password, data.name); };
  return (
    <div className="max-w-md mx-auto">
      <h1 className="font-display text-3xl mb-6">Criar conta</h1>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <input className="w-full px-4 py-3 rounded-lg bg-luxury-black/30 border border-crimson-600/30" placeholder="Nome" {...register('name')} />
